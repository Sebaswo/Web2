...
const userPutCat = (
  ...
  return new Promise((resolve, reject) => {
    const newName = 'Test Cat ' + new Date().toISOString(); //muutin tämän, niin ettei testeissä tehdä kahta samannimistä kissaa peräkkäin..
    request(url)
      .put('/api/v1/cats/' + id)
      .set('Content-type', 'application/json')
      .set('Authorization', 'Bearer ' + token)
...
const getCatByBoundingBox = (url: string | Function): Promise<Cat[]> => {
  return new Promise((resolve, reject) => {
    request(url)
      ...
      .set('Content-type', 'application/json')
      .query({
        topRight: '71,80',    //Muutin koordinaatteja testissä, koska mielestäni tässä ei ollut järkeä. Koodissa tehdyt kissat eivät
        bottomLeft: '10,20',  //olleet boxissa, vaikka koodi niin olettaa: expect(cat.length).toBeGreaterThan(0);
      })
      .expect(200, (err, response) => {
        if (err) {
          reject(err);
